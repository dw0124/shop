<?
	...
	$query= "select no7 from jumun where jumunday7=curdate() order by no7 desc;";
	...
	For ($i=1;  $i<=$n_cart;  $i++)
	{
	   if ($cart[$i]) // 제품정보가 있는 경우만
	   {
		   ...
		   // (주문번호, 제품번호, 수량, 단가, 금액, 할인율, 소옵션번호1,2)
			...
			values ('$o_no' , '$row[no7]', '$num', '$row[price7]', '$cash_price', '$row[discount7]', '$opts1', '$opts2');";
			$result = mysqli_query($db, $query);
			if (!$result) exit("에러: $query");
   ...
   $query = "insert into jumun (no7, member_no7, jumunday7, product_names7, product_nums7, o_name7, o_tel7, o_phone7, o_email7, o_zip7, o_juso7, r_name7, r_tel7, r_phone7, r_email7, r_zip7, r_juso7, memo7, pay_method7, card_okno7, card_halbu7, card_kind7, bank_kind7, bank_sender7, total_cash7, state7)
      values ('$o_no', '$member_no', '$o_jumunday', '$product_names', '$product_nums', '$o_name', '$o_tel', '$o_phone', '$o_email', '$o_zip', '$o_addr', '$r_name', '$r_tel', '$r_phone', '$r_email', '$r_zip', '$r_addr', '$o_etc', '$pay_method', 'aaa', '$card_halbu', '$card_kind', '$bank_kind', '$bank_sender', '$total_price', 1);";
